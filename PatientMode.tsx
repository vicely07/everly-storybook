...
import { compileBookNarrative, generateCaptionFromDetails, transcribeAudio, createCaregiverChat, generateSpeech } from '../services/geminiService';
...
export default function CaregiverMode({ memoryCircle, updateMemoryCircle, onSetupComplete, onExit }: CaregiverModeProps): React.ReactElement {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
        ...
        setPendingInteractions(prev => ({
            ...prev,
            [memoryId]: { status: 'details', file, year: '2005', details: '' }
        }));

        // Generate and play voice instruction
        try {
            const audioDataUri = await generateSpeech("Please add a year and a few details so I can generate a story for this memory.");
            if (audioDataUri) {
  ...
  const generateCaption = async (memoryId: string) => {
      const interaction = pendingInteractions[memoryId];
      if (!interaction) return;

      setPendingInteractions(prev => ({
          ...prev,
          [memoryId]: { ...interaction, status: 'generating' }
      }));

      try {
        const caption = await generateCaptionFromDetails(
            interaction.file, 
            getDisplayName(), 
            interaction.year,
            interaction.details
        ...
        // Update the memory date if year is provided
        if (interaction.year) {
            const date = new Date(`${interaction.year}-01-01`);
            updateMemoryDate(memoryId, date);
        }
        
        finalizeCaption(memoryId, caption);
      } catch (e) {
        console.error(e);
        setPendingInteractions(prev => ({
            ...prev,
                          ...
                          <div className="flex-1 flex flex-col gap-4">
                              <div className="flex items-center gap-3 text-teal-600 mb-6">
                                   ...
                                   <div>
                                       ...
                                       <p className="text-xs text-slate-400">Add a year and a few details to generate a caption.</p>
                                   </div>
                              </div>
                              ...
                              <div className="flex-1 flex flex-col gap-4">
                                  <div>
                                      ...
                                      <input 
                                          ...
                                          className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-teal-500 outline-none text-slate-700 font-bold"
                                          value={interaction.year}
                                          onChange={(e) => setPendingInteractions(prev => ({
                                              ...prev,
                                              [m.id]: { ...interaction, year: e.target.value }
                                          }))}
                                          disabled={interaction.status === 'generating'}
                                      />
                                  </div>
                                  
                                  <div className="flex-1 flex flex-col">
                                      <label className="text-xs font-bold text-slate-400 uppercase mb-1 block">A Little Details</label>
                                      <textarea 
                                          className="flex-1 w-full p-4 rounded-xl border border-slate-200 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 outline-none text-slate-700 text-sm resize-none bg-slate-50 focus:bg-white transition-colors"
                                          placeholder="Who is in this? Where was it? What was happening?"
                                          value={interaction.details}
                                          onChange={(e) => setPendingInteractions(prev => ({
                                              ...prev,
                                              [m.id]: { ...interaction, details: e.target.value }
                                          }))}
                                          disabled={interaction.status === 'generating'}
                                      />
                                  </div>
                                  ...
                                  {interaction.status === 'generating' && (
                                      <div className="flex items-center justify-center gap-2 text-teal-600 font-bold py-2">
                                          <Wand2 className="animate-spin" size={18} /> Writing perfect story...
                                      </div>
                                  )}
                              </div>

                              <button 
                                onClick={() => generateCaption(m.id)} 
                                disabled={interaction.status === 'generating'}
                                className="w-full mt-4 py-4 bg-teal-600 text-white rounded-xl font-bold hover:bg-teal-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed group"
                              >
                                  <Wand2 size={18} className="group-hover:rotate-12 transition-transform"/> 
                                  Auto-Generate Caption & Save
                              </button>
                          </div>
                       ) : (
